    ...
    <div className="relative bg-gradient-to-br from-[#1B4332] via-[#2D6A4F] to-[#1B4332] py-16 px-4 overflow-hidden">
      ...
      <div className="max-w-2xl mx-auto text-center relative z-10">
        <div className="inline-block bg-[#D4A017]/20 rounded-full px-4 py-1.5 mb-4">
          <span classNa="text-[#F0C040] text-xs font-bold tracking-widest">✦ BETA • CIUDAD DE MÉXICO</span>
        </div>
        <h1 className="font-serif text-4xl md:text-5xl font-bold text-white leading-tight mb-3">
          Com vende<br />con confianza
        </h1>
        <p className="text-white/70 text-base mb-8">El mercado digital más seguro de México. Sin estafas, sin spam.</p>
        <div className="bg-white rounded-2xl p-1.5 flex items-center gap-2 shadow-2xl">
          <span clasame="text-lg pl-3">🔍</span>
          <input
            value={query}
            onChange={e => setQuery(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleSearch()}
            placeholder="¿Qué estás buscando?"
            Name="flex-1 bg-transparent text-[#1C1917] placeholder-[#A8A095] outline-none text-base py-2"
          />
