...
export default async function ListingPage({
  ...
  const waPhone = seller?.phone?.replace(/[^0-9]/g, "") ?? "";
  const waText = encodeURIComponent(
    `Hola! Me interesa tu servicio en Naranjogo: "${listing.title_es}" (${price}). ¿Está disponible?`
  );
  ...
  return (
    <main className="min-h-screen bg-[#FDF8F1]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {/* WhatsApp — primary CTA, always visible */}
        {waUrl && (
          <a
            href={waUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full py-4 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 mb-6 shadow-md hover:shadow-lg transition-all hover:brightness-110"
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          <div>
            <p className="text-sm font-semibold text-emerald-800">Transacciones Protegidas</p>
            <p className="text-xs text-emerald-700">
              Chatea por WhatsApp con el proveedor. Los pagos dentro de Naranjogo están protegidos.
            </p>
          </div>
        </div>
