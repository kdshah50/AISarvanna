...
export default async function ListingPage({ params }: { params: { id: string } }) {
  ...
  const res = await fetch(
    `${SUPA_URL}/rest/v1/listings?id=eq.${params.id}&status=eq.active&select=*,users(id,display_name,trust_badge,ine_verified,phone,whatsapp_optin,created_at)`,
  ...
  return (
    <main className="min-h-scree bg-[#FDF8F1]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-3">
          <span className="text-3xl font-bold text-[#1B4332]">{price}<span className="text-base font-semibold text-[#6B7280] ml-2">MXN</span></span>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {listing.shipping_available && <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700">📦 Envío disponible</span>}
          <span className="px-3 py-1 rounded-full text-xs font-medium bg-[#F4F0EB] text-[#6B7280]">{listing.condition}</span>
          {listing.location_city && <span className="px-3 py-1 rounded-full text-xs font-medium bg-[#F4F0EB] text-[#6B7280]">📍 {listing.location_city}</span>}
        </div>
        ...
        {seller && (
          <Link href={`/seller/${seller.id}`} className="block hover:opacity-90 transition-opacity">
            <div className="bg-[#F4F0EB] rounded-xl p-4 mb-6 items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-[#1B4332] flex items-center justify-center text-white text-lg font-bold flex-shrink-0">{seller.display_name?.[0] ?? "V"}</div>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div><p className="text-sm font-semibold text-emerald-800">Compra Protegida</p><p className="text-xs text-emerald-700">Tu pago está protegido hasta confirmar tu compra</p></div>
        </div>
        <div className="flex flex-col gap-3">
          {seller?.whatsapp_optin !== false && (
            <a href={waUrl} target="_blank" rel="noopener noreferrer"
              className="w-full py-4 rounded-xl font-semibold text-base flex items-center justify-center gap-2"
              style={{ background: "#25D366", color: "whit}>
          ...
          <button className="w-full py-4 rounded-xl bg-[#1B4332] text-white font-semibold text-base hover:bg-[#2D6A4F] transition-colors">Contactar vendedor</button>
        </div>
      </div>
    </main>
  );
