...
export default async function SellerPage({ params }: { params: { id: string } }) {
  ...
  const sellerRes = await fetch(
    `${SUPA_URL}/rest/v1/users?id=eq.${ms.id}&select=id,display_name,avatar_url,trust_badge,ine_verified,phone_verified,created_at`,
    { headers: h, cache: "no-store" }
  );
  const [seller] = sellerRes.ok ? await sellerRes.json() : [];
  if (!seller) notFound();
  const listingsRes = await fetch(
    `${SUPA_URL}/rest/v1/listings?seller_id=eq.${params.id}&status=eq.active&order=created_at.desc&select=id,title_es,price_mxn,condition,location_city,photo_urls,shipping_available,negotiable`,
    { headers: h, cache: "no-store" }
  );
  ...
  return (
    <main className="min-h-screen bg-[#FDF8F1]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link href="/" className="inline-flex items-center gap-1 text-sm text-[#6B7280] hover:text-[#1B4332] mb-6 transition-colors">← Volver al inicio</Link>
        <div className="bg-white rounded-3xl border border-[#E5E0D8] p-8 mb-6">
          <div className="flex flex-col items-center text-center gap-4">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#1B4332] to-[#2D6A4F] flex items-center justify-center text-white text-3xl font-ld shadow-lg">{initials}</div>
            <div>
            ...
            <div className="flex gap-4 text-xs text-[#6B7280]">
              <span className={seller.phone_verified ? "text-[#059669]" : ""}>{seller.phone_verified ? "✓" : "○"} Teléfono</span>
              <span className={seller.ine_verified ? "text-[#059669]" : ""}>{seller.ine_verified ? "✓" : "○"} INE</span>
            </div>
            <div clflex gap-3 w-full mt-2">
        ...
        <div className="bg-[#ECFDF5] border border-[#A7F3D0] rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#047857]">Todos los artículos están cubiertos por Compra Protegida.</p>
          </div>
        </div>
        <div>
          <h2 className="fof text-xl font-bold text-[#1C1917] mb-4">Artículos activos <span className="ml-2 text-sm font-normal text-[#6B7280]">({listings.length})</span></h2>
          {listings.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-[#E5E0D8]"><div className="text-5xl mb-3">📦</div><p className="text-[#6B7280]">Sin artículos activos.</p></div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {listings.map((l: any) => (
                <Link key={l.id} href={`/listing/${l.id}`} className="group">
                  <div className="bg-white rounded-2xl overflow-hidden border border-[#E5E0D8] hover:shadow-lg hover:-translate-y-1 transition-auration-200">
                    <div className="relative aspect-[4/3] bg-[#F4F0EB]">
                      {l.photo_urls?.[0] ? <Image src={l.photo_urls[0]} alt={l.title_es} fill className="object-cover" sizes="33vw" /> : <div className="absolute inset-0 flex items-center justify-center text-4xl text-[#E5E0D8]">📦</div>}
                    </div>
                    <div className="p-3">
                      <p className="text-base font-bold text-[#1C1917] mb-0.5">{fmtMXN(l.price_mxn)}</p>
                      <p className="text-xs text-[#374151] line-clamp-2">{l.title_es}</p>
                      {l.locaon_city && <p className="text-[10px] text-[#9CA3AF] mt-1">📍 {l.location_city}</p>}
                    </div>
                  </div>
                </Link>
              ))}
