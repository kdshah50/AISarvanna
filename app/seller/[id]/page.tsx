cd ~import { notFound } from "next/navigation";
...
export async function generateMetadata({
  ...
  return {
    ...
    description: `Perfil del vendedor en Naranjogo.`,
  };
}
...
export default async function SellerPage({
  ...
  const initials = (seller.display_name ?? "V")
    .split(" ")
    .map((w: string) => w[0])
    .join("")
    .slice(0, 2)
  ...
  return (
    <main className="min-h-screen bg-[#FDF8F1]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link
          href="/"
          className="inline-flex items-center gap-1 text-sm text-[#6B7280] hover:text-[#1B4332] mb-6 transition-colors"
        >
          Volver al inicio
        </Link>
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div className="flex gap-4 text-xs text-[#6B7280]">
              <span className={seller.phone_verified ? "text-[#059669]" : ""}>
                {seller.phone_verified ? "+" : "o"} Telefono
              </span>
              <span className={seller.ine_verified ? "text-[#059669]" : ""}>
                {seller.ine_verified ? "+" : "o"} INE
              </span>
            </div>
        ...
        <div className="bg-[#ECFDF5] border border-[#A7F3D0] rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#047857]">
              Todos los articulos de este vendedor estan cubiertos.
            </p>
          </div>
        </div>
          ...
          {listings.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-[#E5E0D8]">
              <p className="text-[#6B7280]">
                Este vendedor no tiene articulos activos.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="bg-white rounded-2xl overflow-hidden border border-[#E5E0D8] hover:shadow-lg transition-all duration-200">
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-xs text-[#374151] line-clamp-2 leading-snug">
                        {listing.title_es}
                      </p>
