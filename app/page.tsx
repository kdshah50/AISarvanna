...
export default async function HomePage({ searchParams }: Props) {
  ...
  try {
    // Always: services only + CP 37745 + optional free text
    let url = `${SUPA_URL}/rest/v1/listings`
      + `?status=eq.active`
      + `&category_id=eq.services`
      + `&zip_code=eq.${SMA_ZIP}`
      + `&select=id,title_es,price_mxn,category_id,condition,location_city`
    ...
    // Free text search — case insensitive match on title
    if (query) {
    ...
    if (res.ok) {
      ...
      cards = Array.isArray(data)
        ? data
            .map((row: any) => {
              const lt = row.location_lat ?? SMA_LAT;
              const ln = row.location_lng ?? SMA_LNG;
              const km = distKm(refLat, refLng, lt, ln);
              return {
                id:                 row.id,
                title:              row.title_es,
  ...
  const heading = query
    ...
    : (lang === "en" ? "Local Services — San Miguel de Allende"
