...
interface Props {
  searchParams?: {
    ...
    lat?: string; lng?: string; lang?: string;
  };
}
...
export default async function HomePage({ searchParams }: Props) {
  ...
  // Use user GPS if provided, otherwise default to SMA center
  ...
  try {
    // Always: services only + CP 37745 + free text search
    let url = `${SUPA_URL}/rest/v1/listings`
      + `?status=eq.active`
    ...
    if (res.ok) {
      ...
      cards = Array.isArray(data) ? data
        .map((row: any) => {
          // Calculate distance from user/SMA center to listing
          const listingLat = row.location_lat ?? SMA_LAT;
          const listingLng = row.location_lng ?? SMA_LNG;
          const km = distKm(refLat, refLng, listingLat, listingLng);
        ...
        // Sort by distance if geo available, otherwise keep DB order
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {hasGeo && (
              <span className="text-xs font-semibold px-3 py-1.5 rounded-full bg-[#ECFDF5] text-[#065F46] border border-[#A7F3D0]">
                📍 {lang === "en" ? "Sorted by distance from you" : "Ordenado por distancia"}
              </span>
            )}
